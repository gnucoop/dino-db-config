CREATE MATERIALIZED VIEW IF NOT EXISTS "public"."location_permissions" AS
SELECT
...
FROM
   ...
   CROSS JOIN jsonb_array_elements_text(ug.location_ref_id) as item
WHERE
   JSONB_ARRAY_LENGTH(ug.location_ref_id) > 0 AND
