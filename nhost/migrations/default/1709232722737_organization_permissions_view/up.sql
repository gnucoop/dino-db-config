CREATE MATERIALIZED VIEW IF NOT EXISTS "public"."organization_permissions" AS
SELECT
   ud.full_name as user_name,
   ud.user_auth_ref_id as user_id,
   ug.id as group_id,
   jsonb_array_elements_text(
      jsonb_agg(
         CASE
            WHEN item = 'all' THEN '7044761a-a850-4c37-b6bc-c2d2ca2a3280' :: uuid
            ELSE item :: uuid
         END
      )
   ) :: uuid as metric_id
FROM
   user_group as ug
   JOIN user_data as ud ON ud.user_group_ids ? ug.id :: text
   CROSS JOIN jsonb_array_elements_text(ug.organization_ref_id) as item
WHERE
   JSONB_ARRAY_LENGTH(ug.organization_ref_id) > 0 AND
   ud.is_deleted = 'false' AND
   ug.is_deleted = 'false'
GROUP BY
   ud.full_name,
   ud.user_auth_ref_id,
   ug.id;
